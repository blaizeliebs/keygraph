...
class KeysoneDataBuilder extends Builder {
  ...
  build() {
    ...
    if (hasDatasource) {
      ...
      let modelName = `${entity.UCFCCSingular}Model`
      let entityName = entity.UCFCCSingular
      if (interfaceName) {
        interfaceData = this.getEntityData(interfaceName)
        modelName = `${interfaceData.entity.UCFCCSingular}Model`
      ...
      if (hasMutations && !isInterface) {
        ...
        code.addBlock(`
          return new Promise((resolve, reject) => {
            ...
            let ${modelName} = keystone.list('${entityName}')
            let where = { _id: ${entity.UCFCCSingular}.objectId(_id) }
...
export default KeysoneDataBuilder
